...
async def clear(ctx, amount=25): #создает функцию очистки последних сообщений в канале
   ...
   для запуска функции нужно написать в чат, в котором нужно очистить сообщения, сначала +clear, затем количество сообщений'''
...
async def BW(ctx): #создает функцию, отправляющую запрещенные на сервере слова
   ...
   аргумент ctx функции отвечает за контекст
   для запуска функции нужно написать в чат +BW'''
...
async def bot_commands(ctx): #создает функцию, отправляющую список всех команд бота
   ...
   аргумент ctx функции отвечает за контекст
   для запуска функции нужно написать в чат +bot_commands'''
...
async def addBW(ctx): #создает функцию, пополняющую файл с запрещенными на сервере словами
   ...
   аргумент ctx функции отвечает за контекст
   для запуска функции нужно написать в чат сначала +addBW, после чего через пробел перечислись все необходимые для добавления в список слова'''
...
async def clearBW(ctx): #создает функцию, очищающую файл с запрещенными словами
   ...
   аргумент ctx функции отвечает за контекст
   для запуска функции нужно написать в чат +clearBW'''
...
async def on_message(message): #создает функцию, проверяющую каждое отправленное сообщение на допустимость
   ...
   message - отправленное пользователем сообщение
   функция активируется всегда при отправке любым пользователем сообщения'''
...
async def createCh(ctx, *channels): #создает функцию для добавления новых текстовых каналов
   ...
   аргумент ctx функции отвечает за контекст
   аргумент *channels принимает все введенные названия каналов
   для запуска функции нужно написать в чат сначала +createCh, после через пробел перечислить названия каналов, которые необходимо добавить'''
...
async def deleteCh(ctx, *channels: discord.TextChannel): #создает функцию для удаления текстовых каналов
   ...
   аргумент ctx функции отвечает за контекст
   аргумент *channels принимает все введенные названия каналов
   для запуска функции нужно написать в чат сначала +deleteCh, после через пробел перечислить названия каналов, которые необходимо удолить'''
...
async def kick(ctx, user: discord.Member): #создает функцию для исключения участников с сервера
   ...
   для запуска функции нужно написать в чат сначала +kick, после имя пользователя, которого нужно кикнуть'''
...
async def rps(ctx):
    ...
    def check(user):  # проверка на тегнутого пользователя, совпадает ли сообщение от произвольного пользователя с тегнутым
        ...
        return str(z) == str(author)  # выводим равен ли дискорд тег + имя, от пользователя, который написал сообщение, автору
    try:  # если сообщение отправляется
        user_choice = await bot_Mess.wait_for('message', timeout=20.0,check=check)  # даем 20 секунд на отправку от автора
    except asyncio.TimeoutError:  # если 20 секунд истекают, а автор не ответил
        await ctx.send(f'{author.mention} не захотел играть с ботом 💔')  # выводим, что автор не откликнулся
    else:
...
async def rusgame(ctx, opponent: discord.Member):
    ...
    вдохновился этим проектом: https://github.com/PapaJoStick/IBOTIK
    '''
    author = ctx.message.author # указывает на автора команды
...
async def add(ctx,a:int,b:int):
    '''Данная функция работает пока бот включен.
    Она очень проста. Складывает два значения введенных после команды +add.
    a:int - число, b:int - число. Числа для данной оперции.
    Данный код взаимствован с данного ресурса: https://quares.ru/?id=459217'''
    await ctx.send(f"{a} + {b} = {a+b}")

@bot_Mess.command()
async def sub(ctx,a:int,b:int):
    '''Данная функция работает пока бот включен.
    Она очень проста. Складывает два значения введенных после команды +sub.
    a:int - число, b:int - число. Числа для данной оперции.
    Данный код взаимствован с данного ресурса: https://quares.ru/?id=459217'''
    await ctx.send(f"{a} - {b} = {a-b}")
...
async def multiply(ctx,a:int,b:int):
    '''Данная функция работает пока бот включен.
    Она очень проста. Складывает два значения введенных после команды +multiply.
    a:int - число, b:int - число. Числа для данной оперции.
    Данный код взаимствован с данного ресурса: https://quares.ru/?id=459217'''
    await ctx.send(f"{a} * {b} = {a*b}")
...
async def divide(ctx,a:int,b:int):
    '''Данная функция работает пока бот включен.
    Она очень проста. Складывает два значения введенных после команды +divide.
    a:int - число, b:int - число. Числа для данной оперции.
    Данный код взаимствован с данного ресурса: https://quares.ru/?id=459217'''
    if b == 0:
        await ctx.send(f'Can"t divide by 0.')
    else:
        await ctx.send(f"{a} / {b} = {a/b}")
...
@bot_Mess.command()
async def roll(ctx, message):   # Запускает функцию по выбору рандомного числа.
    '''Данной функцией могут воспользоваться все пользователи.
    Для запуска функции нажно написать в чат ДС сервера "+roll".
    Бот выведит в чат сообщение с рандомным значение от 1 до 100.
    аргумент ctx функции отвечает за контекст.
    аргумент message функции отвечат за границу максимального рандомного числа.'''
    t = int(message)
    Roll = randint(1,t)   # Выбирает рандомное число.
    await ctx.send(f' Roll: {Roll}')   # Выводит в чат сообщение с рандомным выбранным значением.
    #await ctx.message.delete()   # Удаляет ваше сообщение чтобы не захламлять чат.
...
async def create_role(ctx):   # Вызывает асинхронную функцию по создании роли.
    ...
    а Color - это цвет роли в шестнадцатеричном представлении. Пример: +create_role Bot 0x0ffaa1.
...
async def give_role(ctx):   # Запускает функцию по выдаче ролей.
    ...
    Данная функция добавляет роль. Если пользователь хочет добавить себе роль, то он может добавить её из списка ролей.
    аргумент ctx функции отвечает за контекст'''
